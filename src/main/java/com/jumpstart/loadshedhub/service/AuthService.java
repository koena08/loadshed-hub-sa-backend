...
public class AuthService {
    ...
    public AuthResponseDTO register(AuthRequestDTO r) {
        ...
        User user = users.save(User.builder().email(r.getEmail()).password(encoder.encode(r.getPassword()))
                .firstName(r.getFirstName()).lastName(r.getLastName()).role(Role.ROLE_CITIZEN).build());
        return new AuthResponseDTO(jwt.generateToken(user), user.getEmail(), user.getRole().name());
    }

    public AuthResponseDTO login(AuthRequestDTO r) {
        User user = users.findByEmail(r.getEmail()).orElseThrow(() -> new IllegalStateException("Invalid email or password"));
