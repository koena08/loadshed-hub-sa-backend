...
public class AuthService {
    ...
    public AuthResponseDTO register(AuthRequestDTO r) { if (users.findByEmail(r.getEmail()).isPresent()) throw new IllegalStateException("Email already registered"); Role role = "ROLE_BUSINESS_OWNER".equalsIgnoreCase(r.getRequestedRole()) || "BUSINESS_OWNER".equalsIgnoreCase(r.getRequestedRole()) ? Role.ROLE_BUSINESS_OWNER : Role.ROLE_CITIZEN; User u=users.save(User.builder().email(r.getEmail()).password(encoder.encode(r.getPassword())).firstName(r.getFirstName()).lastName(r.getLastName()).role(role).build()); return response(u); }
